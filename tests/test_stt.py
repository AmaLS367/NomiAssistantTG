...
async def test_ffmpeg_path_invalid_bin():
    ...
    with patch.dict("os.environ", {"FFMPEG_BIN": "/invalid/path"}, clear=False), \
         patch("os.path.isfile", return_value=False), \
...
async def test_get_model_invalid_path():
    ...
    with patch.dict("os.environ", {"VOSK_MODEL_PATH": "/invalid/path"}, clear=False), \
...
async def test_decode_ffmpeg_file_not_found():
    ...
    with patch("stt.vosk_stt._ffmpeg_path", return_value="/nonexistent/ffmpeg"), \
